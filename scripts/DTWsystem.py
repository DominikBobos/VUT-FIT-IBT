...
def RQAunknown(train=None, test=None, feature='mfcc', frame_reduction=5, reduce_dimension=True, second_pass=True, sdtw=True):
    ...
    if not rqa_list:
        for idx, file in enumerate(test[0]):    #rqa analysis first
            ...
            if float(parsed_file[-1]) < 3.0:  # total duration
                rqa_list.append([file, [], frame_reduction, 10000.0])
            ...
            rqa_list.append([file, path, frame_reduction, np.sum(rec_matrix[path])/(int(path[-1][0])-int(path[0][0]))] if path_length > 250 else [file, [], frame_reduction, 5000.0]) #only frames longer than 2.5 seconds are valid
            rqa_time.append(time.time()-start)
    ...
    if second_pass:
        ...
        for idx, file in enumerate(test[0]):
            ...
            if float(parsed_file[-1]) < 3.0:  # total duration
                print("Skipping", file.split('/')[-1], test[1][idx], "because the total duration {} is less than 3.0 seconds".format(parsed_file[-1]))
                continue
            dist_list = []
            have_hit = False
            nested_loop_count = 0
            ...
            for idx_nested, rqa_item in enumerate(rqa_list):
                if rqa_item[1] == [] or len(rqa_item[1])*rqa_item[2] < 450:
                    continue
                if file.split('/')[-1] == rqa_item[0].split('/')[-1]:
                    continue
                # if idx < 109:   # from 250 to the end
                #     continue
                nested_loop_count += 1
                ...
                if sdtw:
                    path = SegmentalDTW(file_nested_array[int(rqa_item[1][0][0])*rqa_item[2]//frame_reduction:int(rqa_item[1][-1][0])*rqa_item[2]//frame_reduction], file_array, R=5, L=300/frame_reduction, dist='cosine')
                    # wp = np.asarray(path[1][3])*frame_reduction not necessary, uncomment when want to know the found path
                    dtw_distance = path[0]
                else:
                    cost_matrix, wp = librosa.sequence.dtw(X=file_array.T, 
                                                        Y=file_nested_array[int(rqa_item[1][0][0])*rqa_item[2]//frame_reduction:int(rqa_item[1][-1][0])*rqa_item[2]//frame_reduction].T, 
                                                        metric='euclidean', 
                ...
                if dtw_distance < hit_threshold:
                    have_hit = True
                    f = open("evalRQA_{}_{}.txt".format("SDTW" if sdtw == True else "DTW", feature), "a")
                ...
                if nested_loop_count > loop_count:
                    break
            # if idx < 109:   # from 250 to the end
            #     print("index:", idx)
            #     continue
        ...
        ft = open("timeRQA_{}_{}.txt".format("SDTW" if sdtw == True else "DTW", feature), "a")
